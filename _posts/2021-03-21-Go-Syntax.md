...
**A tip: If an anonymous `interface` is embedded in a `struct`, this struct can be regarded as an implementation of this `interface` , and any `struct` that implements this interface can be regarded as embedded in this `struct`.**
