...
**A tip: If an anonymous `interface` is embedded in a `struct`, this struct can be regarded as an implementation of this `interface` , and any `struct` that implements this interface can be regarded as contained in this `struct`，including its methods promoted to the outside struct, you can also override some methods.**
